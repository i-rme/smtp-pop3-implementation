...
public class SmtpServerOld extends CustomThread {
	...
	public void run() {
		...
		// Let's check the first command we receive is a HELO command, if not, we close
		// the socket.
		...
		/*
		 * boolean checkUser = false; username =
		 * NetworkUtils.parseMessageRegex("(HELO) ([a-zA-Z0-9]+)", message); checkUser =
		 * server.checkUser(username);
		 * 
		 * if(checkUser) { NetworkUtils.sendMessageCode(250, SMTPMessage(250), output);
		 * }
		 * 
		 * else //550 <SP> <user>: Recipient address rejected: User unknown in virtual
		 * mailbox table. { SMTPCloseConnection(550); }
		 */
		 ...
		 * NetworkUtils.waitMessage(input); NetworkUtils.waitMessage(input);
		 * NetworkUtils.waitMessage(input); NetworkUtils.waitMessage(input);
		 * NetworkUtils.waitMessage(input); NetworkUtils.waitMessage(input);
		 * 
		 * NetworkUtils.sendMessageCode(250, SMTPMessage(250), output);
	...
	public int SMTPCompareCommand(String input) {
		...
		if (NetworkUtils.checkCommand("MAIL", input))
			return 0;

		else if (NetworkUtils.checkCommand("QUIT", input))
			return 1;
